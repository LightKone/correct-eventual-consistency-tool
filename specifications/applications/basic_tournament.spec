...
@invariant
forall p: Player, t: Tournament :: enrollments[p, t] ==> setIn(p, players) && setIn(t, tournaments);


@operations
procedure addTournament(t1: Tournament)
    modifies tournaments;
    ensures setAdd(t1, tournaments, old(tournaments));

procedure remTournament(t1: Tournament)
    modifies tournaments;
    requires !(exists p: Player :: enrollments[p, t1]);
    ensures setRemove(t1, tournaments, old(tournaments));

procedure enroll(p1: Player, t1:Tournament)
    modifies enrollments;
    requires setIn(p1, players) && setIn(t1, tournaments);
    ensures
